...
# Check if configuration values were read successfully
if [ -z "$FW_VERSION" ] || [ -z "$TIMEOUT" ]; then
    echo "Error: Unable to read configuration values from $CONFIG_FILE"
    exit 1
fi
...
CMD="./pppwn --interface enp0s8 --fw $FW_VERSION --stage1 $STAGE1_PAYLOAD --stage2 $STAGE2_PAYLOAD --timeout $TIMEOUT"
